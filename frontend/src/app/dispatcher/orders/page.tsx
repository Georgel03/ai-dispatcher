...
export default function OrdersPage() {
  ...
  const fetchOrders = useCallback(async () => {
    ...
    try {
      ...
      // ATENȚIE: Verifică dacă în backend prefixul routerului pentru comenzi este /api/orders
      ...
      <main className="flex-1 flex flex-col h-full overflow-hidden relative">
        <div className="p-8 flex-1 overflow-y-auto">
          <div className="flex justify-between items-center mb-8">
            <div>
              <h1 className="text-2xl font-bold text-slate-900">Comenzi Curente (Load Board)</h1>
              <p className="text-sm text-slate-500 mt-1">Gestionează mărfurile și pregătește-le pentru dispecerizare.</p>
            </div>
            <button onClick={openAddModal} className="px-5 py-2.5 bg-indigo-600 text-white font-bold text-sm rounded-xl hover:bg-indigo-700 transition-colors shadow-sm flex items-center gap-2">
              <Icon icon="solar:add-circle-bold" width="20" /> Adaugă Comandă
            </button>
          </div>

          {isLoading ? (
            <div className="flex items-center gap-2 text-slate-500"><Icon icon="lucide:loader" className="animate-spin" /> Se încarcă...</div>
          ) : orders.length === 0 ? (
            <div className="bg-white border border-slate-200 border-dashed rounded-2xl p-12 text-center">
               ...
               <h3 className="text-lg font-bold text-slate-800">Nu ai nicio comandă activă</h3>
               <p className="text-slate-500 text-sm mt-2">Apasă pe "Adaugă Comandă" pentru a crea prima cursă.</p>
            </div>
          ) : (
              ...
              {orders.map((order) => (
                <div key={order.id} className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm hover:shadow-md transition-shadow">
                     ...
                     <div>
                       <div className="flex items-center gap-3 mb-2">
                           <span className={`inline-block px-2 py-1 rounded-md text-[10px] font-bold uppercase tracking-wider border ${
                             ...
                             {order.status === 'pending' ? 'În Așteptare (Neasignată)' : order.status}
                           </span>
                           
                           {/* BUTONUL NOU DE ASIGNARE */}
                           {order.status === 'pending' && (
                             ...
                             >
                               <Icon icon="solar:routing-bold" /> Alocă Echipaj
                             </button>
                           )}
                       </div>
                  ...
                  <div className="grid grid-cols-2 gap-4 bg-slate-50 rounded-xl p-4">
                     <div>
                       <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">Greutate</p>
                       <p className="text-sm font-bold text-slate-800">{order.weight_kg.toLocaleString()} kg</p>
                     </div>
                     <div>
                       <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">Tip Remorcă</p>
